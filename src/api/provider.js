...
export default class Provider {
  ...
  createTask(task) {
    ...
    /* На случай временного хранения позаботится и о временном id */
  ...
  sync() {
    if (this._isOnLine()) {
      ...
      return this._api.sync(storeTasks)
        .then((response) => {
          /* Удаляет из хранилища задачи, что были созданы или изменены в оффлайне. Они больше не нужны */
          storeTasks.filter((task) => task.offline).forEach((task) => {
            this._store.removeItem(task.id);
          });

          /* Забирает из ответа синхронизированные задачи */
          const createdTasks = getSyncedTasks(response.created);
          const updatedTasks = getSyncedTasks(response.updated);

          /* Добавляет синхронизированные задачи в хранилище. Хранилище должно быть актуальным в любой момент, вдруг сеть пропадёт */
          [...createdTasks, ...updatedTasks].forEach((task) => {
            this._store.setItem(task.id, task);
          });

          /* Помечает, что всё синхронизировано */
          this._isSynchronized = true;
